...
window.handleGoogleLogin = async (response) => {
    ...
    try {
        const result = await firebaseSignInWithPopup(auth, credential);
        ...
        if (result.additionalUserInfo.isNewUser) {
            ...
            console.log("New Google user data stored in Firestore.");
        } else {
            await firebaseSetDoc(firebaseDoc(db, "users", user.uid), {
        ...
        showMessage(`Google login successful! Welcome, ${user.displayName || user.email}`, false);
        // UI update will be handled by onAuthStateChanged listener
...
firebaseOnAuthStateChanged(auth, (user) => {
    updateUIForAuthState(user); // Call the main UI update function
});
